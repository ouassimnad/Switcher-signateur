...
export function buildSignatureHtml(templateId, data, direction = "rtl") {
  ...
  // Template: Professional Card (with large image on side)
  if (templateId === "professional-card") {
    return `
<table cellpadding="0" cellspacing="0" style="border-collapse:collapse;font-family:'Arial',sans-serif;direction:${dirStyle};width:100%;max-width:600px;border:2px solid #f0f0f0;border-radius:8px;overflow:hidden;">
  <tr>
    <td style="${isRtl ? "padding-left" : "padding-right"}:0;width:220px;">
      ${logoUrl ? `<img src="${logoUrl}" width="220" height="220" style="display:block;width:220px;height:220px;object-fit:cover;" alt="${name}">` : `<div style="width:220px;height:220px;background:#e5e7eb;display:flex;align-items:center;justify-content:center;color:#9ca3af;font-size:48px;">👤</div>`}
    </td>
    <td style="padding:24px;vertical-align:middle;text-align:${textAlign};">
      <div style="color:#1f2937;font-size:18px;font-weight:bold;margin-bottom:4px;">${name}</div>
      <div style="color:#667eea;font-size:13px;font-weight:600;margin-bottom:2px;text-transform:uppercase;letter-spacing:0.5px;">${title}</div>
      <table cellpadding="0" cellspacing="0" style="border-collapse:collapse;width:100%;margin-top:12px;border-top:1px solid #e5e7eb;padding-top:12px;">
        <tr>
          <td style="color:#374151;font-size:12px;padding:4px 0;"><span style="margin-right:8px;">📱</span><a href="tel:${phone.replace(/\s/g, "")}" style="color:#374151;text-decoration:none;">${phone}</a></td>
        </tr>
        <tr>
          <td style="color:#374151;font-size:12px;padding:4px 0;"><span style="margin-right:8px;">✉️</span><a href="mailto:${email}" style="color:#667eea;text-decoration:none;">${email}</a></td>
        </tr>
        <tr>
          <td style="color:#374151;font-size:12px;padding:4px 0;"><span style="margin-right:8px;">🌐</span><a href="${websiteHref}" style="color:#374151;text-decoration:none;">${website}</a></td>
        </tr>
      </table>
      ${socialHtml.length > 0 ? `<div style="margin-top:12px;font-size:11px;display:flex;gap:8px;flex-wrap:wrap;flex-direction:${isRtl ? "row-reverse" : "row"};">${socialHtml.join("")}</div>` : ""}
      ${dotUrl ? `<div style="margin-top:12px;font-size:11px;color:#6b7280;"><img src="${dotUrl}" width="8" height="8" style="vertical-align:middle;margin-right:6px;border-radius:50%;" alt="●"> متاح</div>` : ""}
    </td>
  </tr>
</table>`;
  }
