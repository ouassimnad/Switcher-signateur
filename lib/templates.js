...
function buildSocialColumn(data, mode = "filled", size = 36, gap = 8) {
  const entries = [];
        ...
        return `<tr><td style="padding:0 0 ${gap}px 0;">
          <a href="${href}" target="_blank" style="text-decoration:none;display:block;">
            <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
              <tr><td width="${size}" height="${size}" align="center" valign="middle" style="width:${size}px;height:${size}px;background-color:${s.bg};border:2px solid #8b5cf6;border-radius:10px;text-align:center;vertical-align:middle;">
...
export function buildSignatureHtml(templateId, data, direction = "rtl") {
  ...
  if (templateId === "professional-card") {
    const socialCol = buildSocialColumn(data, "filled", 36, 8);
    return `
<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;direction:${dirStyle};max-width:600px;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;background:#fff;">
  <tr>
    <td style="width:140px;vertical-align:top;padding:0;">
      ${logoUrl
        ...
        : `<div style="width:140px;height:160px;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);display:table;border-radius:${isRtl ? "0 10px 10px 0" : "10px 0 0 10px"};"><div style="display:table-cell;vertical-align:middle;text-align:center;color:#fff;font-weight:700;font-size:14px;padding:16px;">${name}</div></div>`}
    </td>
    <td style="padding:18px 22px;vertical-align:middle;text-align:${textAlign};width:100%;">
      ...
      ${dotUrl ? `<div style="margin-top:12px;"><img src="${dotUrl}" width="8" height="8" style="vertical-align:middle;margin-${isRtl ? "left" : "right"}:6px;border-radius:50%;" alt="●"><span style="font-size:11px;color:#9ca3af;margin-${isRtl ? "right" : "left"}:4px;">متاح للمشاريع</span></div>` : ""}
    </td>
  ...
  if (templateId === "modern-gradient") {
    const socialCol = buildSocialColumn(data, "filled", 36, 8);
    return `
<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;direction:${dirStyle};max-width:550px;border-radius:10px;overflow:hidden;box-shadow:0 2px 8px rgba(0,0,0,0.1);">
  <tr>
      ...
      <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;width:100%;">
        <tr>
          ...
          <td style="padding:24px 16px;vertical-align:middle;text-align:${textAlign};width:100%;">
            ...
            ${dotUrl ? `<div style="margin-top:14px;"><img src="${dotUrl}" width="8" height="8" style="vertical-align:middle;margin-${isRtl ? "left" : "right"}:6px;border-radius:50%;" alt="●"><span style="font-size:11px;color:#9ca3af;margin-${isRtl ? "right" : "left"}:4px;">متاح للمشاريع</span></div>` : ""}
          </td>
...
<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;direction:${dirStyle};max-width:550px;">
  ...
  if (templateId === "minimalist-dark") {
    const socialCol = buildSocialColumn(data, "flat", 32, 8);
    return `
<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;direction:${dirStyle};max-width:550px;background:#111113;border-radius:8px;overflow:hidden;">
  <tr>
  ...
  const socialCol = buildSocialColumn(data, "filled", 36, 8);
  return `
<table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;direction:${dirStyle};max-width:550px;background:linear-gradient(135deg,#fdf2f8 0%,#f5e6ff 100%);border-radius:12px;overflow:hidden;border:1px solid #f9a8d4;">
  <tr>
