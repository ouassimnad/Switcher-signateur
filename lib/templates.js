...
function buildSocialColumn(data, mode = "filled", size = 44, gap = 10) {
  const entries = [];
  ...
  // Make the actual icon a bit larger relative to the container
        ...
        return `<tr><td style="padding:0 0 ${gap}px 0;">
          <a href="${href}" target="_blank" style="text-decoration:none;display:block;">
            <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;">
              <tr><td width="${size}" height="${size}" align="center" valign="middle" style="width:${size}px;height:${size}px;background-color:${s.bg};border-radius:10px;">
...
export function buildSignatureHtml(templateId, data, direction = "rtl") {
  ...
  if (templateId === "professional-card") {
    const socialCol = buildSocialColumn(data, "filled", 44, 10);
    return `
 <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;direction:${dirStyle};max-width:700px;border:1px solid #e[...]
   <tr>
     <td style="width:160px;vertical-align:top;padding:0;">
       ${logoUrl
         ? `<img src="${logoUrl}" width="160" height="180" style="display:block;width:160px;height:180px;object-fit:cover;border-radius:${isRtl ? "0 10px 10px 0" : "10px 0 0 10px"};" alt="${name}"/>`
         : `<div style="width:160px;height:180px;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);display:table;border-radius:${isRtl ? "0 10px 10px 0" : "10px 0 0 10px"};"><div style="display:table-cell;vertical-align:middle;text-align:center;color:#fff;font-weight:700;">${name}</div></div>`}
     </td>
     <td style="padding:18px 22px;vertical-align:middle;text-align:${textAlign};width:100%;">
       ...
       <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-size:12px;color:#4a5568;line-height:1.6;">
         ...
         <tr>
           <td style="padding:3px 0;white-space:nowrap;"><span style="color:#667eea;font-weight:600;${isRtl ? "margin-left:8px;" : "margin-right:8px;"}">✉</span><a href="mailto:${email}" style="color:#4a5568;text-decoration:none;">${email}</a></td>
         </tr>
         <tr>
       ...
       ${dotUrl ? `<div style="margin-top:12px;"><img src="${dotUrl}" width="8" height="8" style="vertical-align:middle;margin-${isRtl ? "left" : "right"}:6px;border-radius:50%;" alt="●"><span style="font-size:12px;color:#7a7a7a;vertical-align:middle">&nbsp;</span></div>` : ""}
     </td>
  ...
  if (templateId === "modern-gradient") {
    const socialCol = buildSocialColumn(data, "filled", 44, 10);
    return `
 <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;direction:${dirStyle};max-width:650px;border-radius:10px;">
   <tr>
       ...
       <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;width:100%;">
         <tr>
           ...
           <td style="padding:24px 16px;vertical-align:middle;text-align:${textAlign};width:100%;">
             ...
             ${dotUrl ? `<div style="margin-top:14px;"><img src="${dotUrl}" width="8" height="8" style="vertical-align:middle;margin-${isRtl ? "left" : "right"}:6px;border-radius:50%;" alt="●"></div>` : ""}
           </td>
  ...
  if (templateId === "professional-light") {
    const socialCol = buildSocialColumn(data, "filled", 44, 10);
    return `
 <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;direction:${dirStyle};max-width:650px;">
   <tr>
       ...
       <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;width:100%;">
         <tr>
           ...
           <td style="padding:20px 16px;text-align:${textAlign};vertical-align:middle;width:100%;">
             ...
             ${dotUrl ? `<div style="margin-top:12px;"><img src="${dotUrl}" width="8" height="8" style="vertical-align:middle;margin-${isRtl ? "left" : "right"}:6px;border-radius:50%;" alt="●"></div>` : ""}
           </td>
  ...
  if (templateId === "minimalist-dark") {
    const socialCol = buildSocialColumn(data, "flat", 40, 10);
    return `
 <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;direction:${dirStyle};max-width:650px;background:#111113;">
   <tr>
  ...
  const socialCol = buildSocialColumn(data, "filled", 44, 10);
  return `
 <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;font-family:'Helvetica Neue',Helvetica,Arial,sans-serif;direction:${dirStyle};max-width:650px;background:linear-gradient(90deg,#fff 0%,#fff 100%);">
   <tr>
     <td style="padding:0;border-${isRtl ? "right" : "left"}:4px solid #7e22ce;">
       <table cellpadding="0" cellspacing="0" border="0" style="border-collapse:collapse;width:100%;">
         <tr>
           ...
           ${socialCol ? `<td style="vertical-align:middle;padding:24px 18px;text-align:center;border-${isRtl ? "right" : "left"}:2px solid #f9a8d4;">${socialCol}</td>` : ""}
         </tr>
       </table>
     </td>
   </tr>
